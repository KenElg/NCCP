...
def init_matrix(dens,T):
    ...
    vol=n/dens
    L=vol**(1/3)
    Rw=np.zeros((n,3),dtype=float)
    R=np.zeros((n,3),dtype=float)
    for x in range(0,rows):
        for y in range(0,rows):
            for z in range (0,rows):
                Rw[i:i+4,:]=np.array([[0+x,0+y,0+z],[0.5+x,0.5+y,0+z],[0.5+x,0+y,0.5+z],[0+x,0.5+y,0.5+z]]) 
                i=i+4
    R=Rw*L/rows
    vs=T*0.5
    V=np.random.normal(0,vs,(n,3))

init_matrix(3E10,300) # Here we plot our R matrix as a scatter plot    
fig = plt.figure()
